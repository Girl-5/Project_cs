...
class Csz_model extends CI_Model {
     ...
     *
     * @param	int aplication_id
     * @return	row_array data
     */
    public function getApplicationAllData($application_id){
       
        $this->db->select('applictn.*, appcnt.name applicant_name, appcnt.phone , appcnt.email,  appcnt.dob, appcnt.nationality, appcnt.city, appcnt.country_of_birth,  appcnt.uploadCv,  appcnt.work_link,
        appcnt.password_link, sex.name user_gender, appcnt.gender, gg.title, gg.contract, gg.description_type ');
        $this->db->from('application AS applictn');
        $this->db->join('applicant AS appcnt', 'applictn.user_id = appcnt.id', 'INNER');
        $this->db->join('gender AS sex', 'appcnt.gender = sex.gender_id', 'INNER');
        $this->db->join('grant AS gg', 'applictn.grant_id = gg.id', 'INNER');
        $this->db->where('applictn.id',$application_id);
        $query = $this->db->get();
