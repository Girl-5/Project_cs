 ...
 * @license	http://astian.org/about-ADPL	ADPL Licenseapp
...
//        die();
        ...
        //if($this->Csz_auth_model->is_group_allowed('grant', 'backend') !== FALSE) $html.= $this->admin_leftli($cur_page, 'grant', 'admin/grant', "Grants", 'fa fa-google');
        
        if($res = $this->Csz_auth_model->get_groups_fromuser($this->session->userdata("user_admin_id"))){
            if($res->user_groups_id == 1){
                ...
                $html.= '<li class="'.$pm_display.'treeview"><a href="#"><i class="glyphicon glyphicon-menu-right"></i> <span>Calls</span><span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a>';
        ...
        //if($this->Csz_auth_model->is_group_allowed('application ', 'backend') !== FALSE) $html.= $this->admin_leftli($cur_page, 'application', 'admin/application', "Applications", 'fa fa-google');
        

            if($cur_page == 'inprogress' || $cur_page == 'submitted'  || $cur_page == 'accepted'  || $cur_page == 'rejected'  ){
                $pm_display = "active ";
            }else{
                $pm_display = "";
            }
            if($cur_page == 'getreviews'){
                $pmreview_display = "active ";
            }else{
                $pmreview_display = "";
            }
            $html.= '<li class="'.$pm_display.'treeview"><a href="#"><i class="glyphicon glyphicon-menu-right"></i> <span>Applications</span><span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a>';
            $html.= '<ul class="treeview-menu">';
                     
            if($res = $this->Csz_auth_model->get_groups_fromuser($this->session->userdata("user_admin_id"))){
                if($res->user_groups_id == 1){
                    
                    if($this->Csz_auth_model->is_group_allowed('admin getreviews', 'backend') !== FALSE) $html.= $this->admin_leftli($cur_page, 'inprogress', 'admin/application/viewallapplications/inprogress', "Incomplete", 'glyphicon glyphicon-chevron-right');
                    if($this->Csz_auth_model->is_group_allowed('admin getreviews', 'backend') !== FALSE) $html.= $this->admin_leftli($cur_page, 'submitted', 'admin/application/viewallapplications/submitted', "Submitted", 'glyphicon glyphicon-chevron-right');
//                    if($this->Csz_auth_model->is_group_allowed('admin getreviews', 'backend') !== FALSE) $html.= $this->admin_leftli($cur_page, 'getreviews', 'admin/getreviews', "Reviewed", 'glyphicon glyphicon-chevron-right');
                  // $html.= $this->admin_leftli($cur_page, 'accepted', 'admin/application/viewallapplications/accepted', "Shortlisted", 'glyphicon glyphicon-chevron-right');
//                    if($this->Csz_auth_model->is_group_allowed('admin getreviews', 'backend') !== FALSE) $html.= $this->admin_leftli($cur_page, 'rejected', 'admin/application/viewallapplications/rejected', "Unsuccessful", 'glyphicon glyphicon-chevron-right');
                }                 
            }

        $html.= $this->admin_leftli($cur_page, 'shortlisted', 'admin/application/viewallapplications/shortlisted', "Shortlisted", 'glyphicon glyphicon-chevron-right');
            $html.= $this->admin_leftli($cur_page, 'preselected', 'admin/application/viewallapplications/preselected', "Pre-Selected", 'glyphicon glyphicon-chevron-right');
            $html.= $this->admin_leftli($cur_page, 'awarded', 'admin/application/viewallapplications/awarded', "Awarded", 'glyphicon glyphicon-chevron-right');
            $html.= '</ul></li>';


        $html.= '<li class="'.$pmreview_display.'treeview"><a href="#"><i class="glyphicon glyphicon-menu-right"></i> <span>Reviews</span><span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a>';
        $html.= '<ul class="treeview-menu">';
        $html.= $this->admin_leftli($cur_page, 'getreviews', 'admin/getreviews', "Reveiwed Applications", 'glyphicon glyphicon-chevron-right');
        //$html.= $this->admin_leftli($cur_page, 'getreviews', 'admin/getreviews', "External Review", 'glyphicon glyphicon-chevron-right');
        //$html.= $this->admin_leftli($cur_page, 'getreviews', 'admin/getreviews', "Final Review", 'glyphicon glyphicon-chevron-right');

        $html.= '</ul></li>';

        $html.= '<li class="'.$pm_display.'treeview"><a href="#"><i class="glyphicon glyphicon-menu-right"></i> <span>Forms</span><span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a>';
        $html.= '<ul class="treeview-menu">';

        $html.= $this->admin_leftli($cur_page, 'forms', 'admin/forms', "Manage Forms", 'glyphicon glyphicon-chevron-right');
        $html.= $this->admin_leftli($cur_page, 'forms', 'admin/forms/new', "Create Form", 'glyphicon glyphicon-chevron-right');
        ...
        if($this->Csz_auth_model->is_group_allowed('applicant ', 'backend') !== FALSE) $html.= $this->admin_leftli($cur_page, 'applicant', 'admin/applicant', "Registered Users", 'fa fa-google');
        ...
        if($this->Csz_auth_model->is_group_allowed('language', 'backend') !== FALSE || 
            ...
            /* Start General Menu */
            if($cur_page == 'users' || $cur_page == 'members' || $cur_page == 'getreviews'  || $cur_page == 'groups' || $cur_page == 'social' || $cur_page == 'settings' || $cur_page == 'lang' || $cur_page == 'upgrade' || $cur_page == 'genlabel' || $cur_page == 'plugin' || $cur_page == 'export'){
                $gel_settings_display = "active ";
            }else{
                $gel_settings_display = "";
            }
           if($this->Csz_auth_model->is_group_allowed('grant', 'backend') !== FALSE) //$html.= '<li class="'.$gel_settings_display.'treeview"><a href="#"><i class="glyphicon glyphicon-menu-right"></i> <span>'.$this->lang->line('nav_gel_settings').'</span><span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a>';
           
           // $html.= '<ul class="treeview-menu">';
            $html.='<li class="'.$gel_settings_display.'">';
            ...
            if($this->Csz_auth_model->is_group_allowed('admin users', 'backend') !== FALSE) $html.= $this->admin_leftli($cur_page, 'users', 'admin/users', "System Administrators", 'fa fa-user');
